...
public class PhotoUploadController {
			...
			)
	{
		...
		//업로드할 파일명
		//원래 파일명으로 저장할 경우 같은 이름으로 업로드 시 덮어써지는 문제 발생
			...
			) 
	{
		...
		//여러 개의 파일들을 업로드(파일명은 뒤에 날짜붙여서 업로드하기)
			...
			@RequestParam("upload") MultipartFile upload)
	{
		...
		//업로드할 파일명
		String uploadFileNmae = FileNameChange.getDateChangeFileName(upload.getOriginalFilename());
		
		//업로드
		try {
			upload.transferTo(new File(uploadPath+"/"+uploadFileNmae));
		} catch (IllegalStateException | IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
